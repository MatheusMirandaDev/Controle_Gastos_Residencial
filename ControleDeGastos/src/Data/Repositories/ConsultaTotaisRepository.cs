...
public class ConsultaTotaisRepository
{
    ...
    public async Task<ReadConsultaTotaisDTO> ObterTotaisGerais()
    {
        ...
        var totalReceitas = await _context
            .Pessoas.SelectMany(pessoa =>
                pessoa
                    .Transacoes.Where(transacao => transacao.Tipo == TipoTransacao.Receita)
                    .Select(transacao => transacao.Valor)
            )
            .SumAsync();

        // Soma das despesas de todas as pessoas
        var totalDespesas = await _context
            .Pessoas.SelectMany(pessoa =>
                pessoa
                    .Transacoes.Where(transacao => transacao.Tipo == TipoTransacao.Despesa)
                    .Select(transacao => transacao.Valor)
            )
            .SumAsync();
